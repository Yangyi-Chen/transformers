...
class LlamaVPreTrainedModel(PreTrainedModel):
...
class LlamaVModel(LlamaVPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.embed_vision_patch = nn.Linear(32*32*3, config.hidden_size, bias=False)
                ...
                if vision_patches is not None and vision_patches.size(0) > 0:
                    ...
                    vision_embeds = torch.cat(
                        [
                            ...
                            torch.zeros(1, self.config.hidden_size).to(vision_embeds.device),  # add a dummy token (for text)
                        ]
...
class LlamaVForCausalLM(LlamaVPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.model = LlamaVModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.diffusion_head = nn.Linear(config.hidden_size, 32 * 32 * 3, bias=False)
    ...
    def self_generate(
        ...
        for _ in tqdm(range(max_length)):
            ...
            output_logits = self.forward(generated_tokens, vision_patches, vision_patch_indices).logits
...
#     LLAMA_START_DOCSTRING,
# )
# class LlamaForSequenceClassification(LlamaVPreTrainedModel):
#     def __init__(self, config):
#         super().__init__(config)
